/// Widget tests for the four panels that consume [StatusMessageView]:
/// search results, navigation tree, multi-pane reader, dictionary sheet.
///
/// These verify each consumer panel's AsyncValue routing (data-empty, error,
/// offline) through a real Riverpod provider tree — the panel-specific copy,
/// icon overrides, and "no raw error / no Retry" guarantees. Error states are
/// non-actionable on purpose (web users can refresh; mobile assets are
/// bundled, so a retry can't fix an inherent failure).
///
/// [StatusMessageView] itself (every variant rendered correctly) and the
/// [statusVariantForError] classification table are covered separately in
/// `test/presentation/widgets/common/status_message_view_test.dart`.
///
/// These pump individual panels with overridden providers and never launch
/// the app or touch the database, so they run as part of the regular
/// `flutter test` suite (no integration_test binding / device needed).
///
/// Run with:
///   flutter test test/presentation/widgets/common/status_message_view_panels_test.dart
library;
...
void main() {
  ...
  group('SearchResultsPanel — fullResults states (specific tab)', () {
    ...
    testWidgets('generic error → error variant, no Retry', (tester) async {
  ...
  group('MultiPaneReaderWidget — content states', () {
    ...
    testWidgets(
        'generic error → errorLoadingContent + clean copy, raw not shown, no Retry',
        (tester) async {
      await _pumpHosted(
        tester,
        const MultiPaneReaderWidget(),
            ...
            dictionaryLookupProvider.overrideWith(
              (ref, DictionaryLookupParams params) async {
                return entriesValue.when(
                  ...
                  loading: () => Future<List<DictionaryEntry>>.delayed(
                      const Duration(seconds: 30), () => const []),
                  error: (e, st) => Future<List<DictionaryEntry>>.error(e, st),
                );
              },
            ),
