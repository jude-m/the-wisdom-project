...
# Override the whole ladder with RESEARCH_MODELS (CSV) or pin the primary with RESEARCH_MODEL.
#
...
def _model_ladder() -> tuple[str, ...]:
    """The generation ladder: RESEARCH_MODELS verbatim, else DEFAULT_MODELS with an
    optional RESEARCH_MODEL pinned to the front (so a single-model override keeps its
    fallback rungs)."""
    explicit = _csv("RESEARCH_MODELS")
    if explicit:
        return tuple(explicit)
    primary = os.environ.get("RESEARCH_MODEL")
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        models=models,
        rewrite_model=os.environ.get("RESEARCH_REWRITE_MODEL") or models[0],
