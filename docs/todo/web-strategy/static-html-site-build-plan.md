...
- ✅ **The trailing-colophon defect is fixed** — detector plus correction,
  ...
  - `lib/domain/slice_alignment.dart` finds them. A slice opening on a label
    that is not a number is misaligned when it holds no running text at all, or
    when the first label below it *is* a number — that second row being the
    leading label the coordinate should have pointed at.
...
- ℹ️ **`FIGURES.headingOnlyLeaves` are not a defect and must not be "fixed".**
  ...
  to a sibling correctly named for them. Moving one would take text from that
