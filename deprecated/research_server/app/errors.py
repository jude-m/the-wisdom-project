"""Structured, safe error envelope for /research (gold-standard plan §3.1–3.2).
...
class ResearchError(Exception):
    """An /research failure already classified into a client-facing envelope.
...
def classify_upstream(exc: Exception) -> ResearchError:
    ...
    if code == 429 or status == "RESOURCE_EXHAUSTED" or "RESOURCE_EXHAUSTED" in text:
        return ResearchError(429, "rate_limited", RATE_LIMITED_MSG, retriable=True)

    # Upstream busy / high demand (all rungs) → 503, retry shortly.
    if code == 503 or status == "UNAVAILABLE" or "UNAVAILABLE" in text:
        return ResearchError(503, "service_unavailable", SERVICE_BUSY_MSG, retriable=True)
    ...
    if (
        ...
        return ResearchError(422, "cannot_answer", CANNOT_ANSWER_MSG, retriable=False)

    # Everything else → generic 502 (Finding #6: no raw exception in the body).
    return ResearchError(502, "server_error", SERVER_ERROR_MSG, retriable=True)
