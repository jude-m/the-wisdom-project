...
| `FIGURES.runLinkChapters` | 505 | chapters named after their group whose own cross-link stands for the whole run — the unfiltered view offers it instead of one link per sutta (`SitePlan.speaksForRun`) |
...
| `FIGURES.readableContainerTocs` | 42 | of those, the ones whose preamble is the book's introduction to the chapter rather than its title, so the page is readable (`textBearingContainerKeys`) |
...
| `FIGURES.nodesWithCommentaryLink` | 13,242 | texts carrying a canon ↔ commentary cross-link, counted where the link is emitted: once per section, one per TOC, and once more per `FIGURES.runLinkChapters` chapter for the run itself. Nodes and not leaves — a TOC links from its container |
...
| `FIGURES.correctedCoordinates` | 88 | leaves whose upstream coordinate did not point at the row their text begins on, in any of the shapes `SliceAlignment` finds — a closing colophon taken for an opening line, a recitation marker, or a coordinate a whole section early — corrected before the tree is used at all (`correctedTreeCoordinates`). Every figure on this page is measured after that correction |
