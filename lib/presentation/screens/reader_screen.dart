...
import '../widgets/ask/ask_button.dart';
...
class _ReaderScreenState extends ConsumerState<ReaderScreen> {
  // ESC handling moved to the app-wide keyboard layer (AppShortcuts +
  ...
  Widget build(BuildContext context) {
    ...
    // ESC dismissal is handled globally now (see AppShortcuts); this widget
    ...
    final searchPanelMaxWidth = (screenWidth - effectiveNavWidth - 400.0).clamp(
        PaneWidthConstants.searchMin, PaneWidthConstants.searchMaxAbsolute);
    ...
    // Drive scrolled-under from our provider, not M3's built-in detection:
    // the built-in is ScrollNotification-based and goes stale across tab
    // switches (a restored jumpTo(0) fires no notification, so the app bar
    // would keep the previous tab's tint).
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: const [
          ...
          // AI Q&A — opens a chat dialog (stub-backed for now)
          AskButton(),
